...
float synth_oscillator_render_with_morph(
    ...
    float morph)
{
    return synth_oscillator_render_with_params(oscillator, sample_rate, oscillator->frequency, morph);
}

// renders and advances the oscillator with temporary frequency/morph controls;
// its stored base frequency and morph are left untouched
float synth_oscillator_render_with_params(synth_oscillator *oscillator,
                                         float sample_rate, float frequency, float morph)
{
    float sample;
    float phase_step;
    ...
    // use the effective pitch to choose the bandlimited table as well as the
    // phase step; using the base pitch for either would make tuning inconsistent
    ...
    // hz / samples-per-second gives cycles per sample wrapping keeps phase in 0..1
