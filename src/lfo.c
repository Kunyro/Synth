...
{
    const float value = synth_lfo_value(lfo, sample_rate);
...
// reads the current waveform value without advancing phase, so simultaneous notes
// capture the same adsr modulation
float synth_lfo_value(const synth_lfo *lfo, float sample_rate)
{
    return synth_wavetable_render(lfo->phase, lfo->frequency_hz, sample_rate, lfo->morph);
}
