...
ALLOCATION_TEST_TARGET := build/test_allocations$(EXEEXT)
MOD_ENVELOPE_TEST_TARGET := build/test_mod_envelope$(EXEEXT)
...
$(MOD_ENVELOPE_TEST_TARGET): tests/test_mod_envelope.c $(CORE_SOURCES) | build
	$(CC) $(CPPFLAGS) $(CFLAGS) $^ -o $@ -lm

$(ALLOCATION_TEST_TARGET): tests/test_allocations.c $(CORE_SOURCES) | build
	$(CC) $(CPPFLAGS) $(CFLAGS) -Dcalloc=synth_test_calloc -Dfree=synth_test_free $^ -o $@ -lm
